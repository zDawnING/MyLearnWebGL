import layer from '../../vendor/utils/layer.js'
 ...
 * @return {[type]}         [description]
 */
export function load (content, transparent) {
  if (content) {
    if(transparent) {
      return layer.open({type: 2, shadeClose: false, content: content, shade: 'background-color: rgba(0,0,0,.5)'})
    }else{
      return layer.open({type: 2, shadeClose: false, content: content, shade: 'background-color: rgba(0,0,0,1)'})
    }
  } else {
